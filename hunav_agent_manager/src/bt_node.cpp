...
namespace hunav
{
      ...
      : Node("hunav_agent_manager")
  {
    ...
    agents_srv_ = this->create_service<hunav_msgs::srv::ComputeAgents>("compute_agents", std::bind(&BTnode::computeAgentsService, this, _1, _2));
  ...
  void BTnode::registerBTNodes()
  {
    ...
    factory_.registerSimpleCondition("RandomChanceCondition", std::bind(&BTfunctions::randomChance, &btfunc_, _1), random_ports);
    factory_.registerSimpleCondition("IsRobotFacingAgent", std::bind(&BTfunctions::robotFacingAgent, &btfunc_, _1), simple_port);
    factory_.registerSimpleCondition("IsAgentVisible", std::bind(&BTfunctions::agentVisible, &btfunc_, _1), agent_visible_ports);
    factory_.registerSimpleCondition("IsRobotClose", std::bind(&BTfunctions::isRobotClose, &btfunc_, _1), proximity_ports);
    factory_.registerSimpleCondition("IsAtPosition", std::bind(&BTfunctions::isAtPosition, &btfunc_, _1), pos_ports);
    factory_.registerSimpleCondition("IsAgentClose", std::bind(&BTfunctions::isAgentClose, &btfunc_, _1), proximity_agent_ports);
