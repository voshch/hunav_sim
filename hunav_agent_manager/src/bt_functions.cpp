...
//}
  ...
  BTfunctions::getVelocityAt(size_t velocity_field_group_id, float x,
    ...
    return {velocity_field[velocity_field_group_id][grid_x][grid_y][0],
            velocity_field[velocity_field_group_id][grid_x][grid_y][1]};
  }

  void BTfunctions::setArenaWorldSize(
      const std::pair<float, float> &arena_world_size) {
    std::lock_guard<std::mutex> lock(arena_world_size_mutex);

    this->arena_world_size = arena_world_size;
  }
  ...
  // // Neu konstruieren mit placement new
